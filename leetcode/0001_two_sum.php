...
// $nums = [6, 8, 2, 11, 7]
// $target = 9
// $map = [6 => 0, 8 => 1, 2 => 2, 11 => 3, 7 => 4]

// $i   $map[$complement]    isset($map[$complement])  [$i, $map[$complement]
// 0     $map[9 - 6] NULL             F                        -
// 1     $map[9 - 8] NULL             F                        -
// 2     $map[9 - 2]  4               T                     [ 2, 4 ]
...
function twoSum($nums, $target)
{
    ...
    for ($i = 0; $i < count($nums); $i++) {
        ...
        if (!empty($map[$complement]) && $map[$complement] !== $i) {
            return [$i, $map[$complement]];
        }
    }
}

print_r(twoSum([6, 8, 2, 11, 7], 9));
